...
export class TouchControls {
  ...
  private createActionButtons() {
    ...
    if (!isHub) {
      // 4. Botão de Mira e Disparo de Flechas (Segure para Mirar 360°, Solte para Disparar)
   ...
   * - Soltar: dispara a flecha na direção exata apontada
   * - Arrastar para o botão Cancelar [X]: cancela sem gastar flecha
   * - Toque rápido: dispara imediatamente na direção atual
   */
  private createAimShootButton(config: {
    ...
    btnContainer.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      ...
      if (this.controller) {
        ...
        // Pega a direção atual que o Roberto está virado no jogo
      ...
      if (this.shootPointerId === pointer.id) {
        ...
        if (this.controller) {
          ...
          if (!wasCancel && wasAiming) {
            ...
            if (dist > 8) {
              // Disparo direcionado com precisão 360° baseado no ângulo mirado
              this.controller.isVirtualShootAimed = true;
              this.controller.virtualShootTriggered = true;
            } else {
              // Toque rápido: dispara imediatamente na direção atual do Roberto
